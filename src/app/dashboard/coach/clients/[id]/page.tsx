  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [
        ...
        supabase.from('journal_entries').select('*').eq('client_id', clientData.profile_id).eq('is_private', false).order('entry_date', { ascending: false }).limit(30),
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
                {journalEntries.map((entry:any) => (
                  <div key={entry.id} style={{ background:t.surface, border:'1px solid '+t.teal+'30', borderRadius:16, padding:'18px 20px' }}>
                    ...
                    <div style={{ fontSize:13, color:t.textDim, lineHeight:1.7, whiteSpace:'pre-wrap' as const }}>{entry.content}</div>
                  </div>
                ))}
              </div>
            )}
