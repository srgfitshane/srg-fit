...
export default function CoachDashboard() {
  ...
  const load = useCallback(async () => {
      ...
      // setActionQueue happens below, after birthdays + digests are loaded,
      // so urgent local signals merge into the same sorted queue.
      ...
      let upcomingBirthdays: Array<{ clientId: string; firstName: string; fullName: string; dob: string; monthDay: string; daysUntil: number }> = []
      ...
      // Fold urgent local signals into the unified queue: birthdays within
      // 3 days and red-flagged weekly digests. Synthesized page-side (not in
      // buildCoachInbox) so the desktop app's inbox is unaffected.
      const localItems: QueueItem[] = [
        ...upcomingBirthdays
          .filter(b => b.daysUntil <= 3)
          .map((b): QueueItem => ({
            id: `birthday-${b.clientId}-${b.monthDay}`,
            type: 'birthday',
            priority: b.daysUntil === 0 ? 84 : 55,
            title: b.daysUntil === 0
              ? `🎂 ${b.firstName}'s birthday is TODAY`
              : `🎂 ${b.firstName}'s birthday ${b.daysUntil === 1 ? 'tomorrow' : `in ${b.daysUntil} days`}`,
            detail: b.monthDay,
            action: 'Shout out',
            color: t.pink,
        ...
        ...((digestData || []) as any[])
          .filter(d => !d.seen_at && Array.isArray(d.flags) && d.flags.some((f: string) => f === 'missed_workouts' || f === 'no_checkin'))
          .map((d): QueueItem => {
            const client = safeClientList.find(c => c.id === d.client_id)
            const name = client?.profile?.full_name || client?.display_name || 'Client'
            const flagBits = (d.flags as string[])
              .map(f => f === 'missed_workouts' ? 'missed workouts' : f === 'no_checkin' ? 'no check-in' : f === 'low_energy' ? 'low energy' : f === 'poor_sleep' ? 'poor sleep' : f === 'new_pr' ? 'new PR' : f)
              .join(' · ')
            return {
              id: `digest-${d.id}`,
              type: 'digest',
              priority: 68,
              title: `${name.split(' ')[0]}'s week needs a look`,
              detail: `${flagBits} · ${d.workouts_done ?? 0} workout${d.workouts_done === 1 ? '' : 's'} this week`,
              action: 'Open client',
              color: t.orange,
              onClick: () => router.push(`/dashboard/coach/clients/${d.client_id}`),
            }
          }),
      ].filter(item => !dismissed.has(item.id))

      setActionQueue([...queueItems, ...localItems].sort((a, b) => b.priority - a.priority))
    ...
    <>
      ...
      <div style={{ background:t.bg, minHeight:'100vh', fontFamily:"'DM Sans',sans-serif", color:t.text }}>
        ...
        <div style={{ padding:28, maxWidth:1200, margin:'0 auto' }} className="coach-pad">
          ...
          {/* Stats density bar removed — it duplicated the pulse chips in the
              client-list header, which are now clickable and carry the same
              numbers. One strip, one source. */}
          ...
          {/* ── UPCOMING BIRTHDAYS ──
              Compact chip strip (was a card grid). Today/soon birthdays also
              surface as queue items above; this strip is the 14-day planning
              view. Each chip deep-links to community with ?shoutout=. */}
          {birthdays.length > 0 && (
            <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' as const, marginBottom:16 }}>
              <span style={{ fontSize:11, fontWeight:800, color:t.textMuted, textTransform:'uppercase' as const, letterSpacing:'0.08em' }}>🎂 Birthdays</span>
              {birthdays.map(b => {
                ...
                return (
                  <button key={b.clientId}
                    ...
                    style={{ background: isToday ? t.orangeDim : t.surfaceUp, border:'1px solid '+(isToday ? t.orange : t.border), borderRadius:999, padding:'4px 12px', fontSize:11, fontWeight:700, color: isToday ? t.orange : t.textDim, cursor:'pointer', fontFamily:"'DM Sans',sans-serif" }}>
                    {b.firstName} · {isToday ? 'TODAY 🎉' : b.daysUntil === 1 ? 'tomorrow' : b.monthDay}
                  </button>
                )
              })}
            </div>
          ...
          {/* ── WEEKLY DIGEST ──
              Collapsed by default: red-flagged digests already surface in the
              queue, so the full grid is a drill-down. */}
          {weeklyDigests.length > 0 && (
            <div style={{ marginBottom:16 }}>
              <button onClick={()=>setDigestsOpen(o=>!o)}
                style={{ width:'100%', display:'flex', alignItems:'center', justifyContent:'space-between', background:t.surface, border:'1px solid '+t.border, borderRadius:12, padding:'10px 14px', cursor:'pointer', fontFamily:"'DM Sans',sans-serif", marginBottom: digestsOpen ? 10 : 0 }}>
                <span style={{ fontSize:11, fontWeight:800, color:t.textMuted, textTransform:'uppercase' as const, letterSpacing:'0.08em' }}>
                  📊 Weekly digests ({weeklyDigests.length}{weeklyDigests.filter((d:any)=>!d.seen_at).length > 0 ? ` · ${weeklyDigests.filter((d:any)=>!d.seen_at).length} unread` : ''})
                </span>
                <span style={{ fontSize:11, color:t.textMuted }}>{digestsOpen ? '▲' : '▼'}</span>
              </button>
              {digestsOpen && (
              <>
            ...
            <div style={{ background:t.surface, border:'1px solid '+t.border, borderRadius:18, overflow:'hidden' }}>
              <div style={{ padding:'16px 24px', borderBottom:'1px solid '+t.border, display:'flex', alignItems:'center', justifyContent:'space-between', gap:8, flexWrap:'wrap' }}>
                <div style={{ display:'flex', alignItems:'center', gap:12, flexWrap:'wrap' }}>
                  ...
                  {/* Pulse stats inline — the single stats strip. Each chip
                      jumps to its surface (Insights opens the panel). */}
                    ...
                    {([
                      {
                        ...
                        go: () => router.push('/dashboard/coach/reviews'),
                      },
                      { label:'Messages', value: unreadMsgs, color:t.teal, go: () => router.push('/dashboard/coach/messages') },
                      { label:'Check-ins', value: pendingCheckins, color:t.orange, go: () => router.push('/dashboard/coach/checkins') },
                      { label:'Insights', value: aiInsights.length, color:t.purple, go: () => setShowInsights(true) },
                      ...(clients.some(c => c.flagged)
                        ? [{ label:'Flagged', value: clients.filter(c => c.flagged).length, color:t.red, go: undefined }]
                        : []),
                    ] as Array<{ label:string; value:number; color:string; go?:()=>void }>).map(item => (
                      <button key={item.label} onClick={item.go} disabled={!item.go}
                        style={{ display:'flex', alignItems:'center', gap:5, background:t.surfaceHigh, border:'1px solid '+t.border, borderRadius:8, padding:'4px 10px', cursor:item.go?'pointer':'default', fontFamily:"'DM Sans',sans-serif" }}>
