...
const PreviewNavIcon = ({ id, active }: { id: string, active: boolean }) => {
  ...
  return (
    <>
      ...
      <div style={{ background:t.bg, minHeight:'100vh', fontFamily:"'DM Sans',sans-serif", color:t.text, display:'flex', flexDirection:'column', maxWidth:480, margin:'0 auto' }}>
        ...
        {/* Nav */}
        <div style={{ height: 68 }} />
        <div style={{ position:'fixed', bottom:0, left:'50%', transform:'translateX(-50%)', width:'100%', maxWidth:480, background:'#0f0f1a', borderTop:'1px solid #252538', display:'flex', alignItems:'center', height:60, zIndex:50, paddingBottom:'env(safe-area-inset-bottom)' }}>
          {NAV.map(n => (
            <button key={n.id} onClick={() => setActiveNav(n.id)}
              ...
              {activeNav === n.id && (
                <div style={{ position:'absolute', top:0, left:'50%', transform:'translateX(-50%)', width:20, height:2.5, borderRadius:2, background:'#00c9b1' }} />
              )}
              <PreviewNavIcon id={n.id} active={activeNav === n.id} />
              <span style={{ fontSize:10, fontWeight:activeNav===n.id?700:500, color:activeNav===n.id?'#00c9b1':'#5a5a78', fontFamily:"'DM Sans',sans-serif" }}>
                {n.label}
              </span>
            </button>
          ))}
        ...
        <div style={{ flex:1, overflowY: activeNav === 'messages' ? 'hidden' : 'auto', padding: activeNav === 'messages' ? 0 : '16px 16px' }}>
          ...
          {/* ── MESSAGES ── */}
          {activeNav === 'messages' && (
            <div style={{ margin:'-16px -16px', height:'calc(100vh - 170px)' }}>
              {coachProfileId ? (
                <RichMessageThread
          ...
          {activeNav === 'resources' && (
            <div style={{ paddingBottom:32 }}>
              <div style={{ fontSize:15, fontWeight:800, marginBottom:16 }}>📚 Resources</div>
              <div style={{ background:t.surface, border:'1px solid '+t.border, borderRadius:14, padding:'28px 20px', textAlign:'center' as const }}>
                <div style={{ fontSize:32, marginBottom:10 }}>📖</div>
                <div style={{ fontSize:14, fontWeight:700, marginBottom:6 }}>Client Resources</div>
                <div style={{ fontSize:13, color:t.textMuted, lineHeight:1.6 }}>Articles, guides, and workouts shared by Coach Shane</div>
              </div>
            </div>
          )}
